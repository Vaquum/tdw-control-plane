...
def _insert_time_cut_window(
    ...
    client.execute(
        ...
        SELECT
            ...
            sum(mean * no_of_trades) / sum(no_of_trades) AS mean,
            sqrt(sum(std * std * no_of_trades) / sum(no_of_trades)) AS std,
            quantileExact(0.5)(median) AS median,
            quantileExact(0.75)(median) - quantileExact(0.25)(median) AS iqr,
            sumKahan(volume) AS volume,
            sum(maker_ratio * no_of_trades) / sum(no_of_trades) AS maker_ratio,
