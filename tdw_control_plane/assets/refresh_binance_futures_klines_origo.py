...
from .create_binance_futures_trades_table_origo import RAW_TABLE_NAME
from .create_origo_database import _get_clickhouse_settings, _make_clickhouse_client
from .daily_futures_trades_to_origo import (
    daily_partitions,
...
def _insert_partition_rows(
    ...
    client.execute(
        ...
        SELECT
            ...
            argMin(price, futures_trade_id) AS open,
            max(price) AS high,
            min(price) AS low,
            argMax(price, futures_trade_id) AS close,
            ...
            argMin(price * quantity, futures_trade_id) AS open_liquidity,
            max(price * quantity) AS high_liquidity,
            min(price * quantity) AS low_liquidity,
            argMax(price * quantity, futures_trade_id) AS close_liquidity,
...
@asset(
    ...
    description='Refreshes the Binance futures 1m kline projection from source-native daily trades',
)
def refresh_binance_futures_klines_origo(
    ...
    try:
        ...
        if existing_count > 0:
            context.log.info(
                f'Found {existing_count} existing Binance futures kline rows for '
