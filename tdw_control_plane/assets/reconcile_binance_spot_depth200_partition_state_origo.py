...
from .create_binance_spot_depth200_1m_table_origo import DEPTH200_1M_TABLE_NAME
...
from .refresh_binance_spot_depth200_1m_origo import refresh_binance_spot_depth200_1m_origo
from .sync_binance_spot_depth200_snapshots_to_origo import (
    DEPTH200_PARTITION_KEY_FORMAT,
...
def _partition_key(minute: datetime) -> str:
    ...
    return minute.astimezone(timezone.utc).strftime(DEPTH200_PARTITION_KEY_FORMAT)
...
def _existing_projection_minutes(client: ClickHouseClient, database: str) -> list[datetime]:
    return _datetime_rows(
        client.execute(
            ...
            FROM {database}.{DEPTH200_1M_TABLE_NAME}
            ORDER BY minute
            """
        )
    )
...
@asset(
    ...
    description='Reconciles Dagster depth200 partition state from existing Origo ClickHouse rows.',
)
def reconcile_binance_spot_depth200_partition_state_origo(
    context: AssetExecutionContext,
) -> dict[str, object]:
    ...
    try:
        ...
        reported_snapshot_count = _report_existing_partitions(
            context,
            asset_key=sync_binance_spot_depth200_snapshots_to_origo.key,
        ...
        reported_projection_count = _report_existing_partitions(
            context,
            asset_key=refresh_binance_spot_depth200_1m_origo.key,
            database=settings.database,
            table_name=DEPTH200_1M_TABLE_NAME,
