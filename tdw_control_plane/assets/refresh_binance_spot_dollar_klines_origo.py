...
from tdw_control_plane.utils.binance_spot_dollar_kline_sql import (
    binance_spot_dollar_kline_projection_sql,
)
...
def _insert_partition_rows(
    ...
    start_datetime, end_datetime = _partition_datetime_bounds(partition_date)
    projection_sql = binance_spot_dollar_kline_projection_sql(
        database=database,
        source_table=RAW_TABLE_NAME,
        datetime_predicate_sql=(
            f"datetime >= toDateTime64('{start_datetime}', 6) "
            f"AND datetime < toDateTime64('{end_datetime}', 6)"
        ),
        dollar_kline_size=DOLLAR_KLINE_SIZE,
    )
    client.execute(
        f"""
