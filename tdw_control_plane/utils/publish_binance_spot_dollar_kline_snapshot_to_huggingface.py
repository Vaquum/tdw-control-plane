...
def _get_binance_spot_dollar_klines(
    *,
    ...
    data = pl.DataFrame(rows, schema=DOLLAR_KLINE_EXPORT_COLUMNS, orient="row")
    if data.height == 0:
        return data
...
This dataset is exported daily from `origo.{DOLLAR_KLINES_TABLE_NAME}` using {resolution_label} dollar bars derived from the 100k Origo dollar-kline foundation.
...
- Source table: `origo.{DOLLAR_KLINES_TABLE_NAME}`
