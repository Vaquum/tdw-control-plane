...
-- 1m book for the UTC day {day:Date}. The session grid is anchored to the day
-- window itself, so window and grid stay aligned whatever timezone the server
-- resolves the naive datetimes in. minutes counts the 1m rows behind each
-- session so a thinly covered session is visible instead of passing as a
-- quiet one.
WITH toDateTime({day:Date}) AS day_start
SELECT
    day_start + 28800 * intDiv(toUnixTimestamp(datetime) - toUnixTimestamp(day_start), 28800) AS session_start,
    count() AS minutes,
